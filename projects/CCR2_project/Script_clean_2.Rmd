...
```{r}
# 1) Put all SCEs in a *named* list
objs <- list(
  sce_11wks                    = sce_11wks,
  sce_20wks                    = sce_20wks,
...
# 2) Define per-object annotations
annot <- data.frame(
  name      = names(objs),
  Condition = c("11wks","20wks","control",
                "GSE123587","GSE154817","GSE252243",
                "GSE116240_LDL_leuko","GSE116240_LDL_foam_cells",
                "sce_GSM4981311","sce"),
  Organ     = c(rep("Aorta", 9), NA_character_), # keep sce's Organ if present
  stringsAsFactors = FALSE
)
rownames(annot) <- annot$name

# 3) Harmonize rows and assays
common_genes  <- Reduce(intersect, lapply(objs, rownames))
# keep only 'counts' to avoid mixing different logNorm strategies
common_assays <- Reduce(intersect, lapply(objs, assayNames))
keep_assays   <- if ("counts" %in% common_assays) "counts" else common_assays

# 4) Normalize structure of each SCE (no .GlobalEnv side effects)
norm_obj <- function(x, nm) {
  x <- x[common_genes, , drop = FALSE]
  # standardize rowData to avoid conflicts
  rowData(x) <- DataFrame(gene_id = common_genes, row.names = common_genes)
  # prune assays
  assays(x)  <- assays(x)[keep_assays]
  # (optional) drop reducedDims; you’ll recompute post-merge/integration
  reducedDims(x) <- SimpleList()
  # set minimal, consistent colData
  cd <- DataFrame(row.names = colnames(x))
  cd$Condition <- annot[nm, "Condition"]
  # keep original Organ for 'sce' if provided; otherwise use annot
  if (!is.null(colData(x)$Organ)) {
    cd$Organ <- colData(x)$Organ
  } else {
    cd$Organ <- annot[nm, "Organ"]
  }
  colData(x) <- cd
  x
}

objs2 <- mapply(norm_obj, objs, names(objs), SIMPLIFY = FALSE)

# 5) Combine
combined_sce <- do.call(cbind, objs2)

# 6) Sanity checks
table(colData(combined_sce)$Condition)
table(is.na(colData(combined_sce)$Organ))
```
...
df<- as.data.frame(Gene_Sets_Complete_updated)
...
cat_map <- tribble(
 ...
 "Mdsc_.Alshetaiwi._et_al._Science_Immunol.2020.", "Neutrophil function", 
  "Coagulation", "Neutrophil function"
)
...
df <- as.data.frame(colData(sce_Neutro))

# Generate scatter plot with your specifications
ggplot(df, aes(x = Ccr2_detected, y = Coagulation, color = Ccr2_detected)) +
  geom_jitter(width = 0.2, alpha = 0.7, size = 4) +
  scale_color_manual(values = c("FALSE" = "gray", "TRUE" = "palegreen4")) +
  labs(
    x = "Ccr2 Detected",
    y = "Coagulation Signature"
  ) +
  theme_classic() +
  theme(
